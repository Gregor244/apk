...
SCAN_POOL = ThreadPoolExecutor(
    max_workers=3 if platform == 'android' else 4,
...
class SharedCache:
    """Thread-safe cache with deepcopy on read/write."""
...
def calculate_volatility_score(price, atr14, gap_pct=0.0, relative_volume=0.0):
    try:
        price = safe_number(price, 0.0)
        atr14 = safe_number(atr14, 0.0)
        gap_pct = abs(safe_number(gap_pct, 0.0))
        relative_volume = safe_number(relative_volume, 0.0)
        atr_pct = (atr14 / price) * 100.0 if price > 0 else 0.0
        return round(min(100.0, atr_pct * 2.0 + gap_pct * 0.75 + min(relative_volume, 5.0) * 5.0), 2)
    except Exception:
        return 0.0
...
def calculate_atr(highs, lows, closes, period=14):
    try:
        highs = [safe_number(x, 0.0) for x in highs if x is not None]
        lows = [safe_number(x, 0.0) for x in lows if x is not None]
        closes = [safe_number(x, 0.0) for x in closes if x is not None]
        ...
        if not trs:
            return 0.0
        period = max(1, min(int(period), len(trs)))
...
def add_cache_items(app, items: dict, visible_symbols=None, keep_symbols=None):
    ...
    with CACHE_LOCK:
        cache = getattr(app, "shared_cache", None)
        old = cache.snapshot() if hasattr(cache, "snapshot") else dict(cache or {})
        new = {}

        for sym in visible_symbols:
            if sym in items:
                new[sym] = copy.deepcopy(items[sym])
            elif sym in old:
                new[sym] = copy.deepcopy(old[sym])

        for sym in keep_symbols:
            if sym in old:
                new[sym] = copy.deepcopy(old[sym])

        if hasattr(cache, "clear") and hasattr(cache, "bulk_update"):
            cache.clear()
            cache.bulk_update(new)
        else:
            app.shared_cache = copy.deepcopy(new)
            cache = app.shared_cache

        # 🔥 Android RAM SAFE LIMIT
        if len(cache) > MAX_CACHE_SIZE:
            trimmed = list(cache.items())[-MAX_CACHE_SIZE:]
            trimmed_dict = {k: copy.deepcopy(v) for k, v in trimmed}
            if hasattr(cache, "clear") and hasattr(cache, "bulk_update"):
                cache.clear()
                cache.bulk_update(trimmed_dict)
            else:
                app.shared_cache = trimmed_dict
...
def enrich_company_meta(sym, data=None):
    ...
    if (name == sym or not name or market_cap <= 0) and sym:
        try:
            prof = safe_json(safe_request(f"https://finnhub.io/api/v1/stock/profile2?symbol={sym}&token={FINNHUB_KEY}", timeout=4))
...
def fetch_ticker_data(ticker):
    ...
    if not ticker:
        return {
            ...
            'pe': "N/A", 'market_cap': 0, 'day_low': 0.0, 'day_high': 0.0, 'year_low': 0.0, 'year_high': 0.0, 'range_position_pct': 0.0,
            'atr14': 0.0, 'relative_volume': 0.0, 'gap_pct': 0.0, 'range_pct': 0.0, 'range_position_pct': 0.0, 'volatility_pct': 0.0
        }
    ...
    try:
        ...
        if rq.status_code == 200:
            q_res = safe_json(rq).get('quoteResponse', {}).get('result', [])
            if q_res:
                q = q_res[0]
                quote_data = q
    ...
    try:
        ...
        if rc.status_code == 200:
            c_res = safe_json(rc).get('chart', {}).get('result', [])
            if c_res:
                ...
                res_data['atr14'] = calculate_atr(highs, lows, closes, 14)
                res_data['relative_volume'] = round((res_data['vol'] / res_data['avg_vol']), 2) if res_data['avg_vol'] > 0 and res_data['vol'] > 0 else 0.0
                if res_data['prev_close'] > 0 and res_data['price'] > 0:
                    res_data['gap_pct'] = round(((res_data['price'] - res_data['prev_close']) / res_data['prev_close']) * 100, 3)
                if res_data['price'] > 0 and res_data['day_high'] > 0 and res_data['day_low'] > 0:
                    res_data['range_pct'] = round(((res_data['day_high'] - res_data['day_low']) / res_data['price']) * 100, 3)
                    res_data['volatility_pct'] = res_data['range_pct']
                    if res_data['day_high'] > res_data['day_low']:
                        res_data['range_position_pct'] = round(((res_data['price'] - res_data['day_low']) / (res_data['day_high'] - res_data['day_low'])) * 100, 2)
    except Exception as exc:
    ...
    try:
        ...
        if ir.status_code == 200:
            result = safe_json(ir).get('chart', {}).get('result', [])
            if result:
                payload = result[0]
    ...
    if res_data['prev_close'] > 0 and res_data['price'] > 0:
        res_data['gap_pct'] = round(((res_data['price'] - res_data['prev_close']) / res_data['prev_close']) * 100, 3)
    if res_data['avg_vol'] > 0 and res_data['vol'] > 0:
        res_data['relative_volume'] = round(res_data['vol'] / res_data['avg_vol'], 2)
    if res_data['day_high'] > res_data['day_low'] and res_data['price'] > 0:
        res_data['range_position_pct'] = round(((res_data['price'] - res_data['day_low']) / (res_data['day_high'] - res_data['day_low'])) * 100, 2)
    ...
    if needs_live_refresh:
        try:
            fh_q = safe_json(safe_request(f"https://finnhub.io/api/v1/quote?symbol={ticker}&token={FINNHUB_KEY}", timeout=4))
            ...
            prof = safe_json(safe_request(f"https://finnhub.io/api/v1/stock/profile2?symbol={ticker}&token={FINNHUB_KEY}", timeout=4))
            if isinstance(prof, dict):
                if prof.get('name'):
            ...
            if res_data['pe'] == "N/A":
                metrics = safe_json(safe_request(f"https://finnhub.io/api/v1/stock/metric?symbol={ticker}&metric=all&token={FINNHUB_KEY}", timeout=4))
...
def ensure_symbol_cache(symbols, app=None, trim=False, visible_symbols=None):
    ...
    missing = []
    for sym in uniq:
        cached = getattr(app, 'shared_cache', {}).get(sym, {})
    ...
    if trim and visible_symbols is not None:
        ...
        with CACHE_LOCK:
            old_cache = getattr(app.shared_cache, 'snapshot', lambda: dict(getattr(app, 'shared_cache', {}) or {}))()
...
def fetch_finnhub_ticker_data(ticker):
    ...
    try:
        metrics = safe_json(safe_request(f"{base_url}/stock/metric", params=params, timeout=5))
        ...
        past_earn = safe_json(safe_request(f"{base_url}/stock/earnings", params=params, timeout=5))
            ...
            if period_str:
                try:
                    ...
                    if react_req.status_code == 200:
                        results = safe_json(react_req).get('chart', {}).get('result', [])
        ...
        earn_cal = safe_json(safe_request(f"{base_url}/calendar/earnings", params={"symbol": ticker, "from": today.strftime('%Y-%m-%d'), "to": future.strftime('%Y-%m-%d'), "token": FINNHUB_KEY}, timeout=5))
        ...
        news = safe_json(safe_request(f"{base_url}/company-news", params={"symbol": ticker, "from": past_week.strftime('%Y-%m-%d'), "to": today.strftime('%Y-%m-%d'), "token": FINNHUB_KEY}, timeout=5))
...
class DataCard(RecycleDataViewBehavior, MDCard):
    text = StringProperty("")
    kind = StringProperty("card")
    markup = BooleanProperty(True)
    button_text = StringProperty("")
    callback = ObjectProperty(None, allownone=True)
    body_color = ListProperty([1, 1, 1, 1])
    ...
    def __init__(self, **kwargs):
        ...
        self.elevation = 1
        self._root = MDBoxLayout(orientation="vertical", spacing=dp(8), size_hint_y=None)
        self._root.bind(minimum_height=self._root.setter('height'))

        self.lbl = MDLabel(text=self.text, markup=self.markup, size_hint_y=None, theme_text_color="Primary", halign="left")
        self.lbl.bind(width=self._sync_text_size)
        self.lbl.bind(texture_size=self._update_height)
        self.lbl.bind(on_ref_press=self._open_ref)

        self.btn = MDRaisedButton(text=self.button_text or "Akcja", size_hint_y=None, height=dp(42), on_release=self._trigger_callback)
        self._root.add_widget(self.lbl)
        self._root.add_widget(self.btn)
        super().add_widget(self._root)
        self.bind(text=self._refresh_view, kind=self._refresh_view, button_text=self._refresh_view, markup=self._refresh_view, callback=self._refresh_view, body_color=self._refresh_view)
        self._refresh_view()

    def refresh_view_attrs(self, rv, index, data):
        self.index = index
        for key in ("text", "kind", "markup", "button_text", "callback", "body_color"):
            if key in data:
                try:
    ...
    def _open_ref(self, instance, ref):
        try:
            if plyer_browser:
                plyer_browser.open(ref)
            else:
                webbrowser.open(ref)
        except Exception as exc:
            print(f"open ref error: {exc}")
        ...
        if callable(self.callback):
            try:
                ...
                try:
                    ...
                    print(f"DataCard callback error: {exc}")
            except Exception as exc:
                print(f"DataCard callback error: {exc}")

    def _refresh_view(self, *args):
        try:
            self.lbl.text = self.text or ""
            self.lbl.markup = bool(self.markup)
            self.btn.text = self.button_text or self.text or "Akcja"
            self.md_bg_color = self.body_color or [1, 1, 1, 1]
            is_button = str(self.kind).lower() == "button"
            self.btn.opacity = 1 if is_button else 0
            self.btn.disabled = not is_button
            self.lbl.opacity = 0 if is_button else 1
            self.lbl.disabled = is_button
            if is_button:
                self.height = dp(42) + dp(24)
            else:
                try:
                    self.height = max(self.lbl.texture_size[1] + dp(24), dp(56))
                except Exception:
                    self.height = dp(80)
        except Exception as exc:
            print(f"DataCard refresh error: {exc}")

    def _update_height(self, instance, size):
        try:
            if str(self.kind).lower() != "button":
                self.lbl.height = size[1]
                self.height = max(size[1] + dp(24), dp(56))
        except Exception:
            pass


class ContentFeed(RecycleView):
    def __init__(self, **kwargs):
        kwargs.setdefault("do_scroll_x", False)
        kwargs.setdefault("do_scroll_y", True)
        super().__init__(**kwargs)
        self.viewclass = DataCard
        self.data = []
        self.scroll_type = ["bars", "content"]
        self.bar_width = dp(8)
        self.effect_cls = ScrollEffect
        layout = RecycleBoxLayout(default_size=(None, dp(120)), default_size_hint=(1, None), size_hint_y=None, orientation="vertical", spacing=dp(8))
        layout.bind(minimum_height=layout.setter("height"))
        self.layout_manager = layout
        super(RecycleView, self).add_widget(layout)

    def _item_from_widget(self, widget):
        if isinstance(widget, DataCard):
            return {"text": widget.text, "kind": widget.kind, "markup": widget.markup, "button_text": widget.button_text, "callback": widget.callback, "body_color": widget.body_color}
        if isinstance(widget, MDLabel):
            return {"text": widget.text, "kind": "label", "markup": getattr(widget, "markup", False)}
        if isinstance(widget, MDRaisedButton):
            return {"text": widget.text, "kind": "button", "button_text": widget.text, "callback": (lambda *a, _w=widget: _w.dispatch('on_release'))}
        if isinstance(widget, dict):
            return dict(widget)
        if hasattr(widget, "text"):
            return {"text": getattr(widget, "text", ""), "kind": "label", "markup": getattr(widget, "markup", False)}
        return {"text": str(widget), "kind": "label", "markup": False}

    def add_widget(self, widget, index=0, canvas=None):
        self.data.append(self._item_from_widget(widget))

    def clear_widgets(self, children=None):
        self.data = []

    def set_items(self, items):
        self.data = copy.deepcopy(list(items or []))

    def add_item(self, **item):
        self.data.append(copy.deepcopy(item))

    def extend_items(self, items):
        self.data.extend(copy.deepcopy(list(items or [])))
...
class ScrollableTab(MDTabsBase, MDBoxLayout):
    ...
    def _compose_items(self, lines):
        items = [{"text": color_wrap(f"Ostatnia aktualizacja: {getattr(self, 'last_update_text', timestamp_text())}", "#888888"), "kind": "label", "markup": True}]
        for line in lines or []:
            if isinstance(line, dict):
                items.append(line)
            else:
                items.append({"text": str(line), "kind": "card", "markup": True})
        return items
    ...
    def _begin_refresh(self, *args, **kwargs):
        if self._loading:
            return
        self._loading = True
        self.content.set_items([{"text": "Pobieranie danych...", "kind": "label", "markup": False}])
    ...
    def _fetch(self, *args, **kwargs):
        try:
            res = safe_request(f"https://finnhub.io/api/v1/stock/market-status?exchange=US&token={FINNHUB_KEY}", timeout=5)
            status_data = safe_json(res)
        ...
        safe_ui(self._render, outputs)

    def _render(self, lines):
        self.content.set_items(self._compose_items(lines))

    def _show_error(self, message):
        self.content.set_items([
            {"text": f"[color=#FF0000][b]{message}[/b][/color]", "kind": "label", "markup": True},
            {"text": "Ponów próbę", "kind": "button", "button_text": "Ponów próbę", "callback": lambda *a: self.refresh_data()},
        ])
...
class InfoTab(ScrollableTab):
    def __init__(self, **kw):
        ...
        row1.add_widget(MDRaisedButton(text="Sprawdź Status Rynków", on_release=lambda x: self.refresh_data(force=True)))
        row1.add_widget(MDRaisedButton(text="Słowniczek", on_release=lambda x: self.toggle_glossary()))
        row1.add_widget(MDRaisedButton(text="TEST Powiadomień", on_release=lambda x: self.run_notification_test(), md_bg_color=(0, 0.5, 0, 1)))
    ...
    def refresh_data(self, *args, **kwargs):
        ...
        if not self._should_refresh_cache(force=force):
            ...
            return
        self.content.set_items([{"text": "Pobieranie statusu rynku...", "kind": "label", "markup": False}])
    ...
    def _render(self, lines):
        items = [
            {"text": color_wrap(f"Ostatnia aktualizacja: {getattr(self, 'last_update_text', timestamp_text())}", "#888888"), "markup": True, "kind": "label"},
            {"text": self._tabs_description_text(), "markup": True, "kind": "card"},
            {"text": "Słowniczek wskaźników (rozwiń / ukryj)" if not self._glossary_visible else "Słowniczek wskaźników (ukryj)", "kind": "button", "button_text": "Słowniczek wskaźników (rozwiń / ukryj)" if not self._glossary_visible else "Słowniczek wskaźników (ukryj)", "callback": self.toggle_glossary},
        ]
        if self._glossary_visible:
            items.append({"text": self._glossary_text(), "markup": True, "kind": "card"})
        for line in lines:
            items.append({"text": line, "markup": True, "kind": "card"})
        self.content.set_items(items)

    def toggle_glossary(self):
        self._glossary_visible = not self._glossary_visible
        if self._status_cache.get("lines"):
            self._render(self._status_cache.get("lines", []))
class SkanerTab(ScrollableTab):
    ...
    def _fetch(self, *args, **kwargs):
        ...
        with CACHE_LOCK:
            cache_snapshot = getattr(app.shared_cache, 'snapshot', lambda: dict(getattr(app, 'shared_cache', {}) or {}))()
    ...
    def _show_error(self, message):
        self.content.set_items([
            {"text": f"[color=#FF0000][b]{message}[/b][/color]", "kind": "label", "markup": True},
            {"text": "Ponów próbę", "kind": "button", "button_text": "Ponów próbę", "callback": self.refresh_data},
        ])
...
class TickerTab(ScrollableTab):
    ...
    def _fetch(self, ticker, *args, **kwargs):
        ...
        vol_trend = "Brak"
        avg_vol = safe_number(data.get('avg_vol', 0.0), 0.0)
        if len(volumes) > 5:
    ...
    def _msg(self, txt, is_card=False):
        if is_card:
            self.content.set_items([{"text": txt, "kind": "card", "markup": True}])
        else:
            self.content.set_items([{"text": txt, "kind": "label", "markup": True}])
        ...
        try:
            ...
            for q in catalyst_queries:
                ...
                if res_news.status_code != 200:
                    continue
                for n in safe_json(res_news).get("news", []):
            ...
            if res_earn.status_code == 200:
                for item in safe_json(res_earn).get('earningsCalendar', []):
            ...
            for info in calendar_data.values():
                ...
                for item in info["raw_items"]:
                    ...
                    data = getattr(app, 'shared_cache', {}).get(sym, {})
                    comp_name = normalize_company_name(sym, data.get('name', sym))
...
class NewsTab(ScrollableTab):
    ...
    def _append_unique(self, item):
        ...
        if not key:
            return False
        if not hasattr(self, "_daily_keys"):
            self._daily_keys = set()
...
class CfdShortTab(ScrollableTab):
    ...
    def _show_error(self, message):
        self.content.set_items([
            {"text": f"[color=#FF0000][b]{message}[/b][/color]", "kind": "label", "markup": True},
            {"text": "Ponów próbę", "kind": "button", "button_text": "Ponów próbę", "callback": lambda: self.refresh_data(force=True)},
        ])
...
class StockScannerPro(MDApp):
    def build(self):
        ...
        self._notification_last_ts = {}
        self._notification_debounce_seconds = 90
    ...
    def _start_background_scheduler(self):
        ...
        if not IS_GITHUB:
            self._scheduler_event = Clock.schedule_interval(self.cron_time_checker, 30)
            self._watchdog_event = Clock.schedule_interval(lambda dt: self.refresh_watchdog(), 15)
    ...
    def refresh_watchdog(self):
        if not self._refresh_active:
            return
        started = getattr(self, "_refresh_started_at", 0.0)
        if not started:
            return
        if time.time() - started > 60:
            print("WATCHDOG RESET REFRESH QUEUE")
            with self._refresh_queue_lock:
                self._refresh_active = False
                self._refresh_active_tab = None
    ...
    def send_notification(self, title, message):
        ...
        key = f"notification|{title}|{message}"
        now = time.time()
        with self._scheduler_lock:
            last = self._notification_last_ts.get(key, 0.0)
            if now - last < self._notification_debounce_seconds:
                return False
            self._notification_last_ts[key] = now
        delivered = False
    ...
    def on_stop(self):
        self._scheduler_running = False
        try:
            if self._scheduler_event is not None:
                self._scheduler_event.cancel()
        except Exception:
            pass
        try:
